...
<div
    class="min-h-screen flex flex-col px-6 py-6
       ...
       dark:bg-[linear-gradient(110deg,#03061B_0%,#3A0363_70%,#260D41_100%)] dark:text-white"
>
    <header class="flex items-center gap-5">
        <a
            class="size-9 flex items-center justify-center rounded-full motion-safe:transition
                ...
                dark:bg-[#4D3076] dark:border-none dark:text-white dark:hover:bg-[#5a3a87]"
    ...
    <main class="flex flex-1 flex-col items-center justify-center text-center">
        ...
        <fieldset class="relative mt-12 w-full max-w-145">
            ...
            <MagicSparkles active={status === "generating"} />
        </fieldset>

        {#if status !== "idle"}
            <MagicCard {answer} visible={status === "revealed"} variant={revealVariant} easterEgg={isEasterEgg} />
        {/if}
    </main>
